...
Both programs accept several options at the command line, pass the __==»--help«==__ option for an overview. Keyboard commands:
 ...
 * __Y__: Disable materials and Gouraud shading. %(small)(_Viewer only_)%
...
(columns2)* "(icon tar){{ page.title }} {{ page.current_release }}, sources, Gzip'd tarball":http://github.com/downloads/klickverbot/{{ page.github_name }}/{{ page.github_name }}-{{ page.current_release }}-source.tar.gz
 * "(icon zip){{ page.title }} {{ page.current_release }}, sources, Zip archive":http://github.com/downloads/klickverbot/{{ page.github_name }}/{{ page.github_name }}-{{ page.current_release }}-source.zip
